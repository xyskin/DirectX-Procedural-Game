							  ...
							  float terrainHeight)
{
	...
	result = m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight, L"../Engine/data/cm.bmp", 300, 300);
	if(!result)
	{
	...
	result = m_Border->Initialize(device, screenWidth, screenHeight, L"../Engine/data/border01.dds", 304, 304);
	if(!result)
	{
