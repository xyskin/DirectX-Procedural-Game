...
bool TextClass::Render(ID3D11DeviceContext* deviceContext, FontShaderClass* FontShader, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix, bool start)
{
	bool result;
	static float r = 0.0f, g = 0.5f, b = 1.0f;
	r += 0.01f;
	g += 0.01f;
	b += 0.01f;
	if (r >= 1)
	...
	if (b >= 1)
		b = 0;
	char strt[23] = "Click vehicle to start";
	result = UpdateSentence(m_sentence12, strt, m_screenWidth / 2 - 50, m_screenHeight / 2 - 200, r, g, b, deviceContext);
	if (!result)
	{
		return false;
	}
