...
bool TerrainClass::InitializeBuffers(ID3D11Device* device)
{
	...
	{
		m_vertices[i].position = D3DXVECTOR3(m_model[i].x, m_model[i].y, m_model[i].z);
		m_vertices[i].texture  = D3DXVECTOR2(m_model[i].tu, m_model[i].tv);
		m_vertices[i].normal   = D3DXVECTOR3(m_model[i].nx, m_model[i].ny, m_model[i].nz);
		m_vertices[i].tangent  = D3DXVECTOR3(m_model[i].tx, m_model[i].ty, m_model[i].tz);
		m_vertices[i].binormal = D3DXVECTOR3(m_model[i].bx, m_model[i].by, m_model[i].bz);
		m_vertices[i].color    = D3DXVECTOR4(m_model[i].r, m_model[i].g, m_model[i].b, 1.0f);
...
void TerrainClass::MidPointCalculate(int xmin, int xmax, int zmin, int zmax)
{
	...
	{
		m_heightMap[zmin * 512 + (xmin + xmax - 1) / 2].y = (m_heightMap[zmin * 512 + xmin].y + m_heightMap[zmin * 512 + xmax].y) / 2;
		m_heightMap[zmin * 512 + (xmin + xmax + 1) / 2].y = (m_heightMap[zmin * 512 + xmin].y + m_heightMap[zmin * 512 + xmax].y) / 2;

		m_heightMap[zmax * 512 + (xmin + xmax - 1) / 2].y = (m_heightMap[zmax * 512 + xmin].y + m_heightMap[zmax * 512 + xmax].y) / 2;
		m_heightMap[zmax * 512 + (xmin + xmax + 1) / 2].y = (m_heightMap[zmax * 512 + xmin].y + m_heightMap[zmax * 512 + xmax].y) / 2;

		m_heightMap[(zmin + zmax - 1) * 512 / 2 + xmin].y = (m_heightMap[zmin * 512 + xmin].y + m_heightMap[zmax * 512 + xmin].y) / 2;
		m_heightMap[(zmin + zmax + 1) * 512 / 2 + xmin].y = (m_heightMap[zmin * 512 + xmin].y + m_heightMap[zmax * 512 + xmin].y) / 2;
...
void TerrainClass::GetTerrainSize(int& screenWidth, int& screenHeight)
{
	screenWidth = m_terrainWidth;
	screenHeight = m_terrainHeight;
	return;
}
